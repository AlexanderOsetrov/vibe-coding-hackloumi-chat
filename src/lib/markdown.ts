...
export function parseMarkdown(text: string): ParsedElement[] {
  ...
  while (currentIndex < text.length) {
    ...
    if (nextBold !== -1)
      nextMarkdownIndex = Math.min(nextMarkdownIndex, nextBold);
    if (nextItalic !== -1)
      nextMarkdownIndex = Math.min(nextMarkdownIndex, nextItalic);
    if (nextCode !== -1)
      nextMarkdownIndex = Math.min(nextMarkdownIndex, nextCode);
        ...
        case "code":
          return React.createElement(
            "code",
            {
              key: index,
              className:
                "bg-zinc-800 text-zinc-300 px-1 py-0.5 rounded text-sm font-mono",
            },
            element.content
          );
        case "text":
